//Worst fit memory management in fixed partitioning

#include<bits/stdc++.h>
using namespace std;

int max_frag;
void insert_worstfit(struct process p,struct block b[],int m){
	max_frag=INT_MIN;
  int id;
	for(j=0;j<m;j++){
		if(p.p_size <= b[j].b_size && b[j].b_flag == 0 && max_frag < (b[j].b_size - p.p_size)){
			max_frag = b[j].b_size - p.p_size;
			id = j;
		}
	}
	if(max_frag != INT_MIN){
