...
public:
    ...
    LJF_process(int id, int AT, int BT)
    {
        this->id = id;
        this->AT = AT;
        this->BT = BT;
...
bool sort_by_arrival_time_LJF(LJF_process p1, LJF_process p2)
{
...
void LJF()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        P.emplace_back(LJF_process(process_id, arrival_time, burst_time));
    }

    // sort the processes on the basis on burst times for FCFS (First Come First Serve)
    sort(P.begin(), P.end(),sort_by_arrival_time_LJF);
